...
async def bot_description(message: types.Message):
    await message.answer('<b>Как оформлять сообщения?</b> \n'
                         ...
                         '<code>1</code> — если клиент утверждает, что оплатил заказ онлайн, а вы его не видите в программе; \n'
                         '<code>2</code> — упал один заказ с онлайн оплатой, а второй, идентичный первому, за наличные; \n'
                         '<code>3</code> — аптека будет закрыта; \n'
                         '<code>4</code> — нет света и нет возможности выставить проблемный статус; \n'
                         '<code>5</code> — курьер на выдаче или клиент у кассы, уточнить снятие проблемы по заказу. \n'
                         '\n'
