...
async def get_users_names(message: types.Message, state: FSMContext):
    ...
    for operator_id in message_text:
        try:
            ...
            dict_statuses.update({operator_id: 'пользователя нет в этом чате или не существует совсем'})
        else:
            if await database.exists_operator_in_dict(schema_name, operator_id):
                dict_statuses.update({member_chat_info['user']['username']: 'уже есть в списке операторов этого чата'})
            else:
                sql = f"""INSERT INTO {schema_name}.{config.TABLE_DICT_OPERATORS} VALUES ({operator_id});"""
                await database.execute(sql, execute=True)
                dict_statuses.update({member_chat_info['user']['username']: 'добавлен в список операторов этого чата'})
    message_report = ''
